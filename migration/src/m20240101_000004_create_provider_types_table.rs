            ...
            .exec_stmt(
                Query::insert()
                    ...
                    .values_panic([
                        ...
                        r#"{"tokens_prompt":{"type":"direct","path":"response.usage.input_tokens","fallback":{"type":"default","value":0}},"tokens_completion":{"type":"direct","path":"response.usage.output_tokens","fallback":{"type":"default","value":0}},"tokens_total":{"type":"expression","formula":"response.usage.total_tokens","fallback":{"type":"expression","formula":"response.usage.input_tokens + response.usage.output_tokens"}},"cache_create_tokens":{"type":"default","value":0},"cache_read_tokens":{"type":"default","value":0}}"#.into(),
                        r#"{"extraction_rules":[{"type":"body_json","path":"model","priority":1,"description":"从请求body提取模型名"},{"type":"query_param","parameter":"model","priority":2,"description":"从query参数提取模型名"}],"fallback_model":"gpt-5"}"#.into(),
                        r#"{"client_id":"app_EMoamEEZ73f0CkXaXp7hrann","redirect_uri":"http://localhost:1455/auth/callback","scopes":"openid profile email offline_access","pkce_required":true,"authorize":{"url":"https://auth.openai.com/oauth/authorize","method":"GET","query":{"client_id":"{{client_id}}","redirect_uri":"{{redirect_uri}}","state":"{{session.state}}","scope":"{{scopes}}","response_type":"code","code_challenge":"{{session.code_challenge}}","code_challenge_method":"S256","id_token_add_organizations":"true","codex_cli_simplified_flow":"true","originator":"codex_cli_rs"}},"exchange":{"url":"https://auth.openai.com/oauth/token","method":"POST","body":{"grant_type":"authorization_code","code":"{{request.authorization_code}}","client_id":"{{client_id}}","redirect_uri":"{{redirect_uri}}","code_verifier":"{{session.code_verifier}}"}},"refresh":{"url":"https://auth.openai.com/oauth/token","method":"POST","body":{"grant_type":"refresh_token","refresh_token":"{{session.refresh_token}}","client_id":"{{client_id}}"}}}"#.into(),
                    ])
                    // Gemini - API Key
                    .values_panic([
                        "gemini".into(),
                    ...
                    .values_panic([
                        ...
                        r#"{"tokens_prompt":{"type":"direct","path":"response.usageMetadata.promptTokenCount","fallback":{"type":"direct","path":"usageMetadata.promptTokenCount"}},"tokens_completion":{"type":"direct","path":"response.usageMetadata.candidatesTokenCount","fallback":{"type":"direct","path":"usageMetadata.completion_tokens"}},"tokens_total":{"type":"expression","formula":"response.usageMetadata.totalTokenCount","fallback":{"type":"expression","formula":"response.usageMetadata.promptTokenCount + response.usageMetadata.candidatesTokenCount"}},"cache_create_tokens":{"type":"default","value":0},"cache_read_tokens":{"type":"default","value":0}}"#.into(),
                        r#"{"extraction_rules":[{"type":"body_json","path":"model","priority":1,"description":"从请求body提取模型名（最高优先级）"},{"type":"url_regex","pattern":"(?:/gemini)?/v1beta/models/([^:/?]+):(?:stream)?[gG]enerateContent","priority":2,"description":"从URL路径提取模型名（支持流式和非流式端点）"},{"type":"url_regex","pattern":"/v1beta/models/([^:/?]+):generateContent","priority":3,"description":"标准generateContent端点模型提取"},{"type":"url_regex","pattern":"/v1beta/models/([^:/?]+):([^/?]+)","priority":4,"description":"v1beta路径参数提取模型名"},{"type":"query_param","parameter":"model","priority":5,"description":"从query参数提取模型名"}],"fallback_model":"gemini-2.5-pro"}"#.into(),
                        r#"{"client_id":"681255809395-oo8ft2oprdrnp9e3aqf6av3hmdib135j.apps.googleusercontent.com","client_secret":"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","redirect_uri":"https://codeassist.google.com/authcode","scopes":"https://www.googleapis.com/auth/cloud-platform","pkce_required":true,"authorize":{"url":"https://accounts.google.com/o/oauth2/v2/auth","method":"GET","query":{"client_id":"{{client_id}}","redirect_uri":"{{redirect_uri}}","state":"{{session.state}}","scope":"{{scopes}}","response_type":"code","code_challenge":"{{session.code_challenge}}","code_challenge_method":"S256","access_type":"offline","prompt":"select_account","include_granted_scopes":"true"}},"exchange":{"url":"https://oauth2.googleapis.com/token","method":"POST","body":{"grant_type":"authorization_code","code":"{{request.authorization_code}}","client_id":"{{client_id}}","client_secret":"{{client_secret}}","redirect_uri":"{{redirect_uri}}","code_verifier":"{{session.code_verifier}}","access_type":"offline","prompt":"select_account","include_granted_scopes":"true"}},"refresh":{"url":"https://oauth2.googleapis.com/token","method":"POST","body":{"grant_type":"refresh_token","refresh_token":"{{session.refresh_token}}","client_id":"{{client_id}}","client_secret":"{{client_secret}}","access_type":"offline","prompt":"select_account","include_granted_scopes":"true"}}}"#.into(),
                    ])
                    // Claude - API Key
                    .values_panic([
                        "anthropic".into(),
                    ...
                    .values_panic([
                        ...
                        r#"{"tokens_prompt":{"type":"direct","path":"usage.input_tokens","fallback":{"type":"direct","path":"usage.prompt_tokens"}},"tokens_completion":{"type":"direct","path":"usage.output_tokens","fallback":{"type":"direct","path":"usage.completion_tokens"}},"tokens_total":{"type":"expression","formula":"usage.total_tokens","fallback":{"type":"expression","formula":"usage.input_tokens + usage.output_tokens"}},"cache_create_tokens":{"type":"direct","path":"usage.cache_creation_input_tokens","fallback":{"type":"direct","path":"usage.prompt_tokens_details.cached_tokens"}},"cache_read_tokens":{"type":"direct","path":"usage.cache_read_input_tokens","fallback":{"type":"direct","path":"usage.cached_tokens"}}}"#.into(),
                        r#"{"extraction_rules":[{"type":"body_json","path":"model","priority":1,"description":"从请求body提取模型名"}],"fallback_model":"claude-4-sonnet"}"#.into(),
                        r#"{"client_id":"9d1c250a-e61b-44d9-88ed-5944d1962f5e","redirect_uri":"https://console.anthropic.com/oauth/code/callback","scopes":"org:create_api_key user:profile user:inference","pkce_required":true,"authorize":{"url":"https://claude.ai/oauth/authorize","method":"GET","query":{"client_id":"{{client_id}}","redirect_uri":"{{redirect_uri}}","state":"{{session.state}}","scope":"{{scopes}}","response_type":"code","code":"true","code_challenge":"{{session.code_challenge}}","code_challenge_method":"S256"}},"exchange":{"url":"https://console.anthropic.com/v1/oauth/token","method":"POST","body":{"grant_type":"authorization_code","code":"{{request.authorization_code}}","client_id":"{{client_id}}","client_secret":"{{session.code_verifier}}","redirect_uri":"{{redirect_uri}}","code_verifier":"{{session.code_verifier}}"}},"refresh":{"url":"https://console.anthropic.com/v1/oauth/token","method":"POST","body":{"grant_type":"refresh_token","refresh_token":"{{session.refresh_token}}","client_id":"{{client_id}}","client_secret":"{{session.code_verifier}}"}}}"#.into(),
                    ])
                    .to_owned(),
            )
            .await?;
