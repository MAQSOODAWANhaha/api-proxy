...
impl PkceVerifier {
    ...
    pub fn with_length(length: usize) -> Self {
        if length < MIN_CODE_VERIFIER_LENGTH || length > MAX_CODE_VERIFIER_LENGTH {
            panic!(
                ...
                MIN_CODE_VERIFIER_LENGTH, MAX_CODE_VERIFIER_LENGTH
            );
        }
    ...
    fn validate_verifier(verifier: &str) -> Result<(), PkceError> {
        ...
        if len < MIN_CODE_VERIFIER_LENGTH || len > MAX_CODE_VERIFIER_LENGTH {
...
impl PkceUtils {
    ...
    /// 从Code Verifier生成Code Challenge（SHA256方法）
    ...
    /// 验证Code Verifier和Code Challenge是否匹配
