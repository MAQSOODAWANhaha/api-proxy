...
use crate::auth::oauth_token_refresh_service::{OAuthTokenRefreshService, RefreshStats, TokenRefreshResult};
...
impl OAuthTokenRefreshTask {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *state = TaskState::Running;
        ...
        stats.next_execution_time = Some(Utc::now() + Duration::seconds(self.config.interval_seconds as i64));
        drop(stats);
        ...
        *self.task_handle.write().await = Some(task_handle);
        ...
        info!("OAuth Token refresh task started with interval {} seconds", self.config.interval_seconds);
        Ok(())
    }
    ...
    pub async fn reload_config(&self, new_config: RefreshTaskConfig) -> Result<()> {
        let _ = self.control_sender.send(TaskControl::ReloadConfig(new_config));
    ...
    /// 执行刷新任务
    async fn execute_refresh_task(refresh_service: &Arc<OAuthTokenRefreshService>) -> Result<Vec<TokenRefreshResult>> {
        ...
        match &result {
            Ok(refresh_results) => {
                ...
                info!("OAuth Token refresh task completed in {:?}: {} successful, {} failed", 
                      duration, successful_count, failed_count);
            },
            Err(e) => {
                error!("OAuth Token refresh task failed after {:?}: {:?}", duration, e);
            }
        }
    ...
    async fn update_task_stats(
        ...
        if stats.total_executions == 1 {
            ...
            stats.average_execution_duration_ms = 
                (stats.average_execution_duration_ms * (stats.total_executions - 1) as f64 + duration_ms) 
        ...
        match execution_result {
            Ok(refresh_results) => {
                ...
                // 统计刷新结果
                let successful_refreshes = refresh_results.iter().filter(|r| r.success).count() as u64;
