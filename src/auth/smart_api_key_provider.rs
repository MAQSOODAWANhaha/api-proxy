...
impl SmartApiKeyProvider {
    ...
    pub async fn get_valid_credential(&self, provider_key_id: i32) -> Result<CredentialResult> {
        debug!("Getting valid credential for provider_key_id: {}", provider_key_id);
        
        // 1. 先检查缓存
        if let Some(cached) = self.get_cached_credential(provider_key_id).await? {
            debug!("Found cached credential for provider_key_id: {}", provider_key_id);
        ...
        match provider_key.auth_type.as_str() {
            "api_key" => {
                ...
                // 缓存API密钥（永不过期）
                self.cache_credential(provider_key_id, CachedCredential {
            ...
            auth_type => {
                ...
                Err(ProxyError::authentication(format!("Unsupported auth_type: {}", auth_type)))
            }
        }
    }
    ...
    async fn handle_oauth_credential_with_refresh_service(
        ...
        if let Some(cached) = self.get_cached_credential(provider_key_id).await? {
            debug!("Found cached credential after lock for provider_key_id: {}", provider_key_id);
            return Ok(CredentialResult {
                credential: cached.credential,
                auth_type: cached.auth_type,
                refreshed: false,
                ...
                if refresh_result.success {
                    ...
                    if let Some(new_access_token) = refresh_result.new_access_token {
                        ...
                        match self.oauth_client.get_valid_access_token(oauth_session_id).await {
                            ...
                            Ok(None) => {
                                warn!("No valid OAuth access token available for provider_key_id: {}", provider_key_id);
                                self.fallback_to_api_key(provider_key)
                            },
                            Err(e) => {
                                error!("Failed to get OAuth access token for provider_key_id: {}: {:?}", provider_key_id, e);
                                self.fallback_to_api_key(provider_key)
                            }
                        }
                    }
                    ...
                    // 刷新失败，尝试降级
                    warn!("OAuth token refresh failed for provider_key_id: {}, error: {:?}", 
                          provider_key_id, refresh_result.error_message);
                    self.fallback_to_api_key(provider_key)
                }
            },
            Err(e) => {
                error!("OAuth refresh service error for provider_key_id: {}: {:?}", provider_key_id, e);
                self.fallback_to_api_key(provider_key)
            }
        }
    }
    ...
    /// 降级到使用存储的API密钥
    fn fallback_to_api_key(&self, provider_key: &user_provider_keys::Model) -> Result<CredentialResult> {
        if !provider_key.api_key.is_empty() {
            warn!("Falling back to stored api_key for provider_key_id: {}", provider_key.id);
            Ok(CredentialResult {
                credential: provider_key.api_key.clone(),
            ...
            Err(ProxyError::authentication("OAuth token refresh failed and no fallback API key available".to_string()))
        }
    }
    
    /// 检查缓存中的凭证是否有效
    async fn get_cached_credential(&self, provider_key_id: i32) -> Result<Option<CachedCredential>> {
                ...
                AuthCredentialType::OAuthToken { .. } => {
                    ...
                    if let Some(expires_at) = cached.expires_at {
                        ...
                        if Utc::now() + buffer_time < expires_at {
                            ...
                            debug!("Cached OAuth token expired or expiring soon for provider_key_id: {}", provider_key_id);
                        }
                    }
                }
            }
    ...
    /// 从数据库加载provider key
    async fn load_provider_key_from_db(&self, provider_key_id: i32) -> Result<user_provider_keys::Model> {
        ...
        user_provider_keys::Entity::find()
            ...
            .one(&*self.db)
            .await
            .map_err(|e| ProxyError::database_with_source(format!("Failed to load provider key: {:?}", e), e))?
            .ok_or_else(|| ProxyError::authentication(format!("Provider key not found or inactive: {}", provider_key_id)))
    }
