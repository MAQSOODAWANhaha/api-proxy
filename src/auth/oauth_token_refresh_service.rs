...
impl OAuthTokenRefreshService {
    ...
    pub async fn passive_refresh_if_needed(&self, session_id: &str) -> Result<TokenRefreshResult> {
        ...
        // 执行被动刷新
        self.refresh_token_with_lock(session_id, RefreshType::Passive).await
    }
    ...
    pub async fn active_refresh_expiring_tokens(&self) -> Result<Vec<TokenRefreshResult>> {
        ...
        for session in expiring_sessions {
            ...
            // 执行主动刷新
            match self.refresh_token_with_lock(&session.session_id, RefreshType::Active).await {
                Ok(result) => {
                    if result.success {
                        info!("Successfully refreshed token for session: {}", session.session_id);
                    } else {
                        warn!("Failed to refresh token for session: {}, error: {:?}", 
                              session.session_id, result.error_message);
                    }
                    results.push(result);
                },
                Err(e) => {
                    error!("Error refreshing token for session {}: {:?}", session.session_id, e);
        ...
        info!("Active refresh completed, processed {} sessions", results.len());
        Ok(results)
    }
    ...
    async fn should_refresh_token(&self, session_id: &str) -> Result<bool> {
        let session = oauth_client_sessions::Entity::find()
            ...
            .one(&*self.db)
            .await
            .map_err(|e| ProxyError::database_with_source(format!("Failed to find OAuth session: {:?}", e), e))?
            .ok_or_else(|| ProxyError::authentication(format!("OAuth session not found: {}", session_id)))?;
        ...
        debug!("Session {} expires at {:?}, should refresh: {}", 
    ...
    /// 使用锁进行token刷新，防止并发刷新
    async fn refresh_token_with_lock(&self, session_id: &str, refresh_type: RefreshType) -> Result<TokenRefreshResult> {
        // 获取刷新锁
        ...
        if refresh_type == RefreshType::Passive && !self.should_refresh_token(session_id).await? {
            debug!("Token already refreshed by another thread for session: {}", session_id);
            return Ok(TokenRefreshResult {
                success: true,
                new_access_token: None,
                new_expires_at: None,
        ...
        // 执行实际的token刷新
        let result = self.perform_token_refresh(session_id, refresh_type.clone()).await;
    ...
    /// 执行实际的token刷新
    async fn perform_token_refresh(&self, session_id: &str, refresh_type: RefreshType) -> Result<TokenRefreshResult> {
        debug!("Performing token refresh for session: {}, type: {:?}", session_id, refresh_type);
        ...
        match self.oauth_client.get_valid_access_token(session_id).await {
            ...
            Err(e) => {
                error!("Failed to refresh token for session {}: {:?}", session_id, e);
                Ok(TokenRefreshResult {
                    success: false,
                    new_access_token: None,
                    new_expires_at: None,
    ...
    async fn find_expiring_oauth_sessions(&self) -> Result<Vec<oauth_client_sessions::Model>> {
        ...
        oauth_client_sessions::Entity::find()
            ...
            .all(&*self.db)
            .await
            .map_err(|e| ProxyError::database_with_source(format!("Failed to find expiring OAuth sessions: {:?}", e), e))
    }
    ...
    async fn get_token_expires_at(&self, session_id: &str) -> Option<DateTime<Utc>> {
        match oauth_client_sessions::Entity::find()
            ...
            .await
        {
            Ok(Some(session)) => Some(DateTime::<Utc>::from_naive_utc_and_offset(session.expires_at, Utc)),
            _ => None,
        }
    }
...
impl OAuthTokenRefreshServiceBuilder {
    ...
    /// 构建刷新服务
    pub fn build(self, db: Arc<DatabaseConnection>, oauth_client: Arc<OAuthClient>) -> OAuthTokenRefreshService {
