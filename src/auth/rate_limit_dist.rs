...
impl DistributedRateLimiter {
    ...
    /// 简单的每日请求限制（自然日）
    pub async fn check_per_day(&self, user_id: i32, endpoint: &str, limit: i64) -> Result<DistRateLimitOutcome> {
        // 将 endpoint 继续复用，若需更细粒度可在外层区分
        let date = chrono::Utc::now().format("%Y%m%d").to_string();
        let key = format!("{}:{}", CacheKeyBuilder::rate_limit(user_id, endpoint).build(), date);
        ...
        Ok(DistRateLimitOutcome { allowed: current <= limit, current, limit, ttl_seconds: ttl as i64 })
    }
}
...
mod tests {
    ...
    async fn smoke_test_memory_backend() {
        ...
        for i in 1..=3 {
            ...
            last = Some(out);
            if i <= 2 { assert!(last.as_ref().unwrap().allowed); } else { assert!(!last.as_ref().unwrap().allowed); }
        }
    }
}
