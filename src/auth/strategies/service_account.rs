...
impl ServiceAccountStrategy {
    ...
    pub fn new(private_key: String, client_email: String, token_uri: Option<String>) -> Self {
        Self {
    ...
    pub fn from_service_account_key(key_content: &str) -> Result<Self, AuthError> {
        ...
        let token_uri = key_data
            .get("token_uri")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());

        let project_id = key_data
            .get("project_id")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
    ...
    pub async fn get_access_token_with_assertion(
        ...
        let token_response: Value =
            serde_json::from_str(&body).map_err(|e| AuthError::JsonError(e))?;
    ...
    fn parse_token_response(&self, response: &Value) -> Result<OAuthTokenResult, AuthError> {
        ...
        let expires_in = response.get("expires_in").and_then(|v| v.as_i64());
...
mod tests {
    ...
    fn test_service_account_strategy_creation() {
        ...
        let strategy = ServiceAccountStrategy::new(private_key.clone(), client_email.clone(), None);
    ...
    fn test_from_service_account_key() {
        ...
        let strategy =
            ServiceAccountStrategy::from_service_account_key(&key_json.to_string()).unwrap();
        assert_eq!(
            strategy.client_email,
    ...
    fn test_validate_service_account_key() {
        ...
        assert!(
            ServiceAccountStrategy::validate_service_account_key(&valid_key.to_string()).is_ok()
        );
        ...
        assert!(
            ServiceAccountStrategy::validate_service_account_key(&invalid_key.to_string()).is_err()
        );
        ...
        assert!(
            ServiceAccountStrategy::validate_service_account_key(&wrong_type_key.to_string())
                .is_err()
        );
    }
