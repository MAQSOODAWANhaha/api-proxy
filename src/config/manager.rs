...
impl ConfigManager {
    ...
    pub async fn from_file(config_path: impl AsRef<Path>) -> crate::error::Result<Self> {
                        ...
                        while let Ok(event) = event_receiver.recv().await {
                            match event {
                                ConfigEvent::Reloaded(new_config) => {
                                    ...
                                    if let Err(e) = Self::apply_env_overrides(&mut final_config, &env_overrides_clone) {
                                        warn!("应用环境变量覆盖失败: {}", e);
                                    } else {
        ...
        info!("配置管理器初始化完成");
        info!("- 热重载: {}", if watcher.is_some() { "启用" } else { "禁用" });
    ...
    fn apply_override_to_config(
        ...
        match parts.as_slice() {
            ...
            ["redis", "pool", "size"] | ["redis", "poolsize"] => {
                config.redis.pool_size = value.parse()
                    .map_err(|e| crate::error::ProxyError::config_with_source(
...
domains = ["localhost"]
    ...
    async fn test_env_override() {
        env::set_var("PROXY_SERVER_PORT", "9090");
        env::set_var("PROXY_SERVER_WORKERS", "8");
...
domains = ["localhost"]
        ...
        // 清理环境变量
        env::remove_var("PROXY_SERVER_PORT");
        env::remove_var("PROXY_SERVER_WORKERS");
    }
}