...
use sea_orm::{DatabaseConnection, QuerySelect, entity::*, query::*};
...
pub async fn get_overview(
    ...
    get_unified_overview(State(state), Query(unified_query), headers).await
}
...
pub async fn get_request_stats(
    ...
    get_detailed_traces(State(state), Query(unified_query), headers).await
}
...
pub async fn get_unified_overview(
    ...
    let mut response_times: Vec<i64> = traces.iter().filter_map(|t| t.duration_ms).collect();
    ...
    response::success(overview).into_response()
}
...
pub async fn get_health_overview(
    ...
    response::success(health_overview).into_response()
}
...
pub async fn get_dashboard_cards(
    ...
    let response_times: Vec<i64> = traces.iter().filter_map(|t| t.duration_ms).collect();
    let avg_response_time = if !response_times.is_empty() {
        response_times.iter().sum::<i64>() as f64 / response_times.len() as f64
    } else {
        0.0
...
pub async fn get_dashboard_trend(
    ...
    response::success(serde_json::Value::Array(trend_data)).into_response()
}
...
pub async fn get_provider_distribution(
    ...
    response::success(serde_json::Value::Array(distribution)).into_response()
}
...
pub async fn get_request_logs(
    ...
    response::paginated(logs, pagination).into_response()
}
...
pub async fn get_response_time_analysis(
    ...
    // 整体统计
    let mut all_response_times: Vec<i64> = traces.iter().filter_map(|t| t.duration_ms).collect();
...
pub async fn get_request_log_detail(
    ...
    response::success(detail).into_response()
}
...
pub async fn get_today_dashboard_cards(
    ...
    let yesterday_start = (now - Duration::days(1)).date_naive().and_hms_opt(0, 0, 0).unwrap();
    ...
    let tokens_today: i64 = today_traces.iter()
        .map(|t| t.tokens_total.unwrap_or(0) as i64)
        .sum();
    
    let response_times: Vec<i64> = today_traces.iter()
        .filter_map(|t| t.duration_ms)
        .collect();
    ...
    let tokens_yesterday: i64 = yesterday_traces.iter()
        .map(|t| t.tokens_total.unwrap_or(0) as i64)
        .sum();
    
    let response_times_yesterday: Vec<i64> = yesterday_traces.iter()
        .filter_map(|t| t.duration_ms)
        .collect();
    ...
    let rate_response_time = calculate_growth_rate(avg_response_time_today, avg_response_time_yesterday);
...
pub async fn get_tokens_trend(
    ...
    // 计算今天、平均值和最大值
    let today_traces: Vec<&proxy_tracing::Model> = traces.iter()
        .filter(|t| {
            let trace_date = DateTime::<Utc>::from_naive_utc_and_offset(t.created_at, Utc).date_naive();
            trace_date == Utc::now().date_naive()
        })
        .collect();
    
    let current_token_usage: i64 = today_traces.iter()
        .map(|t| t.tokens_total.unwrap_or(0) as i64)
        .sum();
...
pub async fn get_user_api_keys_request_trend(
        ...
        .await
    {
        Ok(traces) => traces,
        Err(err) => {
            tracing::error!("Failed to fetch traces for user API keys request trend: {}", err);
...
pub async fn get_user_api_keys_token_trend(
        ...
        .await
    {
        Ok(traces) => traces,
        Err(err) => {
            tracing::error!("Failed to fetch traces for user API keys token trend: {}", err);
...
/// 解析时间范围参数
fn parse_time_range(query: &TimeRangeQuery) -> Result<(DateTime<Utc>, DateTime<Utc>), axum::response::Response> {
