...
impl HealthCheckService {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        if *running {
            return Err(ProxyError::server_init(
                "Health check service already running".to_string(),
            ));
        }
    ...
    pub async fn add_server(
        ...
        self.health_status
            .write()
            .await
            .insert(server_address.clone(), health_status);
    ...
    pub async fn get_overall_health(&self) -> anyhow::Result<HealthCheckStatistics> {
        ...
        let avg_response_time = if !health_status.is_empty() {
            let total_ms: u64 = health_status
                .values()
    ...
    pub async fn manual_check(&self, server_address: &str) -> Result<HealthCheckResult> {
        ...
        if let Some(task) = task {
            let result = self
                .checker
                .check_health(&task.server_address, &task.config)
                .await?;
            self.update_health_status(&task.server_address, result.clone())
                .await?;
            Ok(result)
        } else {
            Err(ProxyError::server_init(format!(
                "Server {} not found in health monitoring",
                server_address
            )))
        }
    }
        ...
        for (task, check_result) in results {
            match check_result {
                Ok(health_result) => {
                    // 更新健康状态
                    if let Err(e) = self
                        .update_health_status(&task.server_address, health_result.clone())
                        .await
                    {
                        tracing::error!(
                ...
                Err(e) => {
                    ...
                    if let Err(update_err) = self
                        .update_health_status(&task.server_address, error_result)
                        .await
                    {
                        tracing::error!(
    ...
    async fn update_health_status(
        ...
        if let Some(status) = health_map.get_mut(server_address) {
            ...
            tracing::warn!(
                "Attempted to update health status for unknown server: {}",
                server_address
            );
        }
    ...
    pub async fn cleanup_old_results(&self, max_age: Duration) -> Result<usize> {
        ...
        for status in health_map.values_mut() {
            ...
            status
                .recent_results
                .retain(|result| result.timestamp > cutoff_time);
    ...
    pub async fn get_statistics(&self) -> HealthCheckStatistics {
        ...
        let active_tasks = tasks_map
            .values()
            .filter(|t| matches!(t.status, TaskStatus::Pending | TaskStatus::Running))
            .count();

        // 计算平均响应时间
        let total_response_time: u64 = health_map
            .values()
            .map(|s| s.avg_response_time.as_millis() as u64)
            .sum();
...
mod tests {
    ...
    async fn test_add_remove_server() {
        ...
        assert!(
            service
                .add_server(server_addr.clone(), provider_id, None)
                .await
                .is_ok()
        );
    ...
    async fn test_manual_health_marking() {
        ...
        assert!(
            service
                .mark_server_unhealthy(&server_addr, "Test".to_string())
                .await
                .is_ok()
        );
    ...
    async fn test_get_healthy_unhealthy_servers() {
        ...
        let provider_id = ProviderId::from_database_id(1);
        service
            .add_server("server1:8080".to_string(), provider_id.clone(), None)
            .await
            .unwrap();
        service
            .add_server("server2:8080".to_string(), provider_id.clone(), None)
            .await
            .unwrap();

        // 标记一个为不健康
        service
            .mark_server_unhealthy("server1:8080", "Test".to_string())
            .await
            .unwrap();
