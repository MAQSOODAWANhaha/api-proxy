...
impl CacheClient {
        ...
        T: Serialize,
    {
        ...
        self.set_with_ttl(key, &json_value, self.config.default_ttl).await
    }
    
    /// 设置缓存值并指定 TTL
    pub async fn set_with_ttl(&self, key: &str, value: &str, ttl_seconds: u64) -> Result<()> {
        debug!("设置缓存: key={}, ttl={}s", key, ttl_seconds);
    ...
    pub async fn delete(&self, key: &str) -> Result<bool> {
        ...
        debug!("缓存删除结果: key={}, deleted={}", key, was_deleted);
        Ok(was_deleted)
    }
    
    /// 检查缓存是否存在
    pub async fn exists(&self, key: &str) -> Result<bool> {
        debug!("检查缓存存在性: key={}", key);
        ...
        debug!("缓存存在性检查结果: key={}, exists={}", key, exists);
        Ok(exists)
    }
    
    /// 设置缓存过期时间
    pub async fn expire(&self, key: &str, ttl_seconds: u64) -> Result<bool> {
        debug!("设置缓存过期时间: key={}, ttl={}s", key, ttl_seconds);
        ...
        let success: bool = conn
            ...
            .await
            .map_err(|e| ProxyError::cache_with_source(&format!("设置缓存过期时间失败: {key}"), e))?;
            
        debug!("缓存过期时间设置结果: key={}, success={}", key, success);
        Ok(success)
    }
    
    /// 获取缓存剩余存活时间
    pub async fn ttl(&self, key: &str) -> Result<i64> {
        debug!("获取缓存剩余存活时间: key={}", key);
        ...
        debug!("缓存剩余存活时间: key={}, ttl={}s", key, ttl);
        Ok(ttl)
    }
    
    /// 批量删除符合模式的缓存
    pub async fn delete_pattern(&self, pattern: &str) -> Result<u64> {
        warn!("批量删除缓存: pattern={}", pattern);
        ...
        let keys: Vec<String> = conn
            .keys(pattern)
            .await
            .map_err(|e| ProxyError::cache_with_source(&format!("查找匹配的缓存键失败: {pattern}"), e))?;
            
        if keys.is_empty() {
            debug!("没有找到匹配的缓存键: {}", pattern);
            return Ok(0);
        }
        ...
        let deleted_count: i32 = conn
            .del(&keys)
            .await
            .map_err(|e| ProxyError::cache_with_source(&format!("批量删除缓存失败: {pattern}"), e))?;
            
        warn!("批量删除缓存完成: pattern={}, deleted={}", pattern, deleted_count);
    ...
    pub async fn ping(&self) -> Result<()> {
        ...
        if response == "PONG" {
            ...
            error!("Redis ping 响应异常: {}", response);
