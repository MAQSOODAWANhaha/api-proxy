...
impl CacheKey {
    ...
    pub fn pattern(&self) -> String {
        match self {
            ...
            CacheKey::Config { .. } => "config:*".to_string(),
            CacheKey::RateLimit { user_id, .. } => {
                format!("ratelimit:{}:*", user_id)
            }
            CacheKey::ProviderConfig { .. } => "provider:config:*".to_string(),
            CacheKey::AuthToken { .. } => "auth:token:*".to_string(),
    ...
    pub fn is_temporary(&self) -> bool {
        matches!(
            self,
            CacheKey::UserSession { .. } | CacheKey::AuthToken { .. } | CacheKey::RateLimit { .. }
        )
    }
    ...
    pub fn is_config(&self) -> bool {
        matches!(
            self,
            CacheKey::Config { .. } | CacheKey::ProviderConfig { .. }
        )
    }
