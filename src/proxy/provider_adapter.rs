...
/// 提供商特定的请求修改器
pub trait ProviderRequestModifier {
    /// 修改请求头
    fn modify_headers(&self, headers: &mut std::collections::HashMap<String, String>);
    
    /// 修改请求体
    fn modify_body(&self, body: &mut Value) -> Result<()>;
}

/// OpenAI请求修改器
pub struct OpenAIRequestModifier;

impl ProviderRequestModifier for OpenAIRequestModifier {
    fn modify_headers(&self, headers: &mut std::collections::HashMap<String, String>) {
        // 确保使用正确的Content-Type
    ...
    fn modify_body(&self, body: &mut Value) -> Result<()> {
        ...
        if let Some(obj) = body.as_object_mut() {
            ...
            if !obj.contains_key("max_tokens") {
                obj.insert("max_tokens".to_string(), Value::Number(serde_json::Number::from(4096)));
            }
        }
        Ok(())
    }
...
/// Gemini请求修改器
pub struct GeminiRequestModifier;

impl ProviderRequestModifier for GeminiRequestModifier {
    fn modify_headers(&self, headers: &mut std::collections::HashMap<String, String>) {
        // Gemini使用不同的认证头
        if let Some(auth_header) = headers.get("Authorization").cloned() {
        ...
        if let Some(obj) = body.as_object_mut() {
            ...
            if let Some(messages) = obj.remove("messages") {
                if let Some(messages_array) = messages.as_array() {
                    let contents = messages_array.iter().map(|msg| {
                        serde_json::json!({
                            "parts": [{
                                "text": msg.get("content").unwrap_or(&Value::String("".to_string()))
                            }]
                        })
                    }).collect::<Vec<_>>();
