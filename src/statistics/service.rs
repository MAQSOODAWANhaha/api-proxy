...
impl StatisticsService {
    ...
    pub async fn finalize_and_extract_stats(
        ...
        if ctx.response_details.is_sse_format() {
            // 使用新的 Usage+Model 统一流式收口
            let mut stats = crate::statistics::usage_model::finalize_streaming(ctx);

            // 费用计算（有模型与 provider）
            if let (Some(model), Some(provider)) =
                (stats.model_name.clone(), ctx.provider_type.as_ref())
            {
                if let Ok((cost, currency)) = crate::statistics::price::calculate(
                    &self.pricing_calculator,
                    &model,
                    provider.id,
                    &stats.usage,
        ...
        // 非 JSON 内容类型直接跳过解析，避免无意义告警与开销
        if let Some(ref ct) = ctx.response_details.content_type {
            if !crate::statistics::util::content_type_is_json(ct) {
                tracing::info!(
                    component = "statistics.service",
                    request_id = %ctx.request_id,
                    content_type = %ct,
                    "stats_skip: non-JSON content-type; using default values"
                );
        ...
        let decoded: Cow<[u8]> =
            crate::statistics::util::decompress_for_stats(encoding, raw, 512 * 1024); // 512KB 上限
        ...
        // 进一步清理和验证响应体
        // 简化处理：直接基于原始字符串进行解析
        let cleaned_body_str = body_str.to_string();
        ...
        match serde_json::from_str::<serde_json::Value>(&cleaned_body_str) {
            Ok(json) => {
                tracing::debug!(component = "statistics.service", request_id = %ctx.request_id, "Successfully parsed JSON for stats");
...
impl StatisticsService {
    ...
    pub async fn extract_stats_from_json(
        ...
        // 使用合并后的使用/模型解析（统一实现）
        stats.model_name = crate::statistics::usage_model::extract_model_from_json(json);
        stats.usage = crate::statistics::usage_model::extract_tokens_from_json(
