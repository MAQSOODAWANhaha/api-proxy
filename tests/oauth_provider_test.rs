...
use api_proxy::auth::oauth_client::providers::{ApiKeyConfig, ProviderConfigBuilder};
...
mod tests {
    ...
    async fn test_oauth_provider_config_creation() {
        ...
        let _manager = ApiKeyConfig::new(std::sync::Arc::new(db));
        // 只要能够顺利创建管理器，即视为通过
    }
    ...
    async fn test_oauth_url_generation_no_duplicate_params() {
        let db = create_test_db().await;
        let manager = ApiKeyConfig::new(std::sync::Arc::new(db));
