...
> 现有规范偏向桌面端数据面板。本节补齐移动端展示规则，避免“横向滚动 + 信息堆叠”导致的可用性问题。

### 7.1 布局优先级

- 小屏优先保证 **可读性** 与 **可操作性**：按钮宽度、触控区域、表格信息密度要可控。
- 允许在移动端将同一份数据以“卡片列表（Card List）”替代表格网格展示，但语义样式类仍需一致（见 4.2）。

### 7.2 图表（Chart）

- 图表容器在移动端必须显式定义高度（推荐 220–280px），避免依赖固定 `aspect-video` 导致图表过矮。
  - 推荐：`ChartContainer` 使用 `aspect-auto` 并设置 `h-[240px] sm:h-[280px]`。
- Legend 在移动端必须支持换行：`flex-wrap` + `gap-*`，避免横向溢出。
- Tooltip 保持“轻边框 + 无阴影”（遵守 4.4），避免浮层噪音。

### 7.3 表格（Table）

- 默认（`sm` 及以上）继续使用标准表格：`DataTableShell + Table`。
- 移动端（`sm` 以下）推荐切换为卡片列表：
  - 每条记录一张卡片：`rounded-xl` + `border` + `bg-white`，行间用 `gap` 或 `divide-y` 表达层级。
  - 保留关键字段：时间 / 方法+路径 / 状态 / 模型 / Token / 费用 / 耗时。
  - 其他字段（如 User-Agent、IP、Token 拆分）使用折叠区（`details/summary`）或二级区域展示，避免首屏过长。
  - Request ID 建议用 `table-code`（允许轻阴影）并支持截断。
