...
数据表的详细结构、字段和索引由 `sea-orm-migration` 管理。

**所有数据表结构定义的唯一真实来源是 `migration/` 目录下的迁移文件。**

以下是关键数据表的简要说明：

- **users**: 存储用户信息。
- **provider_types**: 存储AI服务商的配置信息。
- **user_provider_keys**: 存储用户持有的、指向具体AI服务商的密钥（即密钥池）。
- **user_service_apis**: 存储用户对外暴露的、统一的API服务及其策略配置。
- **oauth_client_sessions**: 存储OAuth 2.0流程中的会话信息和令牌。
- **proxy_tracing**: 存储所有请求的详细追踪日志，用于统计和分析。
...
所有API接口的详细设计、请求/响应格式、字段说明和示例均已迁移至 `/api` 目录下的独立 Markdown 文档中。

请参考 `/api` 目录以获取最新和最准确的API文档。
}
